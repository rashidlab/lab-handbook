...
[Open OnDemand](https://ondemand.rc.unc.edu) provides a web-based interface to Longleaf—no SSH client required. This is often the easiest way to get started.
...
::: {.callout-note}
You must have a Longleaf account before using OnDemand. See [Request Access](#longleaf-setup) above.
:::
...
### Using OnDemand for Development with Git

OnDemand + RStudio provides a complete development environment:

#### Initial Setup (One Time)

1. Launch RStudio Server via OnDemand
2. Open Terminal tab in RStudio (or use Shell Access)
3. Configure Git:

```bash
git config --global user.name "Your Name"
git config --global user.email "your_onyen@unc.edu"
```
...
2. **Open your project** (File → Open Project, or navigate via Files pane)
3. **Pull latest changes:**
   ```bash
   git pull origin main
   ```
4. **Work on code** in RStudio editor
5. **Test interactively** in R console
6. **Commit and push:**
   ```bash
   git add -A
...
If sessions consistently fail:

1. Go to **Jobs → Active Jobs**
2. Delete the problematic job
3. Clear browser cache
4. Try launching again

For persistent issues, contact [ITS Research Computing](https://help.rc.unc.edu/).
