...
## Run→Diagnose→Fix Loop {#run-diagnose-fix}
...
Claude immediately:
1. Sees the missing package error
2. Checks DESCRIPTION or the install instructions in CLAUDE.md
3. Installs the package (or tells you how)
...
Commit after each fix so you have a clean history of what was wrong and how it was resolved. See [Commits as Checkpoints](daily-workflow.qmd#commit-checkpoints).
:::
...
Claude runs the check, reads NOTEs/WARNINGs/ERRORs, and fixes them — missing documentation, failed tests, namespace issues. This is especially powerful combined with [TDD](daily-workflow.qmd#writing-tests): write tests first, then iterate on the implementation until all checks pass.
