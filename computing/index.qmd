...
Keep your code and documentation in sync. All three lab templates ([Research Project](../templates/research-project.qmd), [Methods Paper](../templates/methods-paper.qmd), and Clinical Trial) include a `CLAUDE.md` that references the consistency framework, so Claude already knows the expected structure when you start a session. However, the templates provide the directory layout but **not the actual framework files** — after cloning a template, ask Claude to generate them for your project (see below).
...
See [Template Status](../project-consistency/claude-code-enforcement.qmd#template-status) for exactly which files each template includes.
:::
...
- **Quota**: 1 TB shared across the lab — check usage at [service.rc.unc.edu](https://service.rc.unc.edu/)
...
- **Access changes**: Email [research@unc.edu](mailto:research@unc.edu) to add/remove lab members (group: `rc_nur2lab_psx`)
- **Compliance**: If storing NIST 800-171 regulated data, see [Longleaf Compliance](https://help.rc.unc.edu/longleaf-compliance)

See [Lab Policies](../policies/index.qmd) for data management and HIPAA requirements, or [Longleaf Setup](../onboarding/setup-longleaf.qmd) for the full directory reference.
